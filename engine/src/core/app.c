...
static nds_result create_demo_scene(nds_instance** out_root)
{
    ...
    *out_root = nds_instance_create(NDS_CLASS_DATAMODEL, "DemoWorld");
    ...
    memset(&p, 0, sizeof(p)); p.anchored=1; p.can_collide=1; p.visible=1;
    p.position=(nds_vec3){0,-1,0}; p.size=(nds_vec3){12,1,8}; p.color_rgba=0x4a6fa1ff; nds_part_set_properties(floor,&p);
...
{
    nds_result rc; nds_config* cfg; platform_window_desc win_desc; nds_instance* scene=NULL; nds_draw_list draw_list;
    nds_gles2_renderer* renderer=NULL; nds_mesh_cache* mesh_cache=NULL; nds_texture_cache* texture_cache=NULL; nds_camera camera;
    nds_player_controller player; nds_round round; nds_disaster_system disasters; int render_width,render_height;
    nds_clock clock; unsigned long frames_run=0; const int smoke_frames=options?options->smoke_test_frames:0; int elimination_logged=0;
    if(!options)return NDS_ERR_INVALID_ARG;
    rc=platform_init();if(rc!=NDS_OK){NDS_LOGE(TAG,"platform_init failed (%d)",(int)rc);return rc;}
    ...
    memset(&win_desc,0,sizeof(win_desc));win_desc.title=options->window_title?options->window_title:"Natural Disaster Survival - Local Runtime";
    win_desc.width=options->window_width>0?options->window_width:nds_config_get_int(cfg,"window.width",1024);win_desc.height=options->window_height>0?options->window_height:nds_config_get_int(cfg,"window.height",768);win_desc.resizable=1;
    rc=platform_create_window(&win_desc);if(rc!=NDS_OK){nds_config_destroy(cfg);platform_shutdown();return rc;}
    nds_draw_list_init(&draw_list);nds_camera_init(&camera);nds_round_init(&round);nds_disaster_system_init(&disasters);
    rc=nds_mesh_cache_create(&mesh_cache,256);if(rc!=NDS_OK)goto cleanup_window;rc=nds_texture_cache_create(&texture_cache,256);if(rc!=NDS_OK)goto cleanup_mesh;
    ...
    {nds_gles2_desc render_desc={win_desc.width,win_desc.height,camera.fov_y_degrees,camera.near_plane,camera.far_plane};rc=nds_gles2_renderer_create(&renderer,&render_desc);}
    if(rc!=NDS_OK)goto cleanup_scene;render_width=win_desc.width;render_height=win_desc.height;nds_clock_init(&clock);nds_perf_reset();
    NDS_LOGI(TAG,"OpenGL rendering backend active; WASD moves, SPACE jumps");
    while(!platform_quit_requested()){
        double dt; nds_round_state previous_round_state;
        if(platform_poll_events()!=0)platform_request_quit();if(platform_is_key_down(PLATFORM_KEY_ESCAPE))platform_request_quit();nds_perf_begin_frame();dt=nds_clock_tick(&clock);
        ...
        if(round.state==NDS_ROUND_PLAYING&&player.alive){
            nds_disaster_system_update(&disasters,&player,scene,(float)dt);
            nds_player_update(&player,scene,(float)dt,platform_is_key_down(PLATFORM_KEY_W),platform_is_key_down(PLATFORM_KEY_S),platform_is_key_down(PLATFORM_KEY_A),platform_is_key_down(PLATFORM_KEY_D),platform_is_key_down(PLATFORM_KEY_SPACE));
        ...
        if(rc!=NDS_OK){NDS_LOGE(TAG,"render failed (%d)",(int)rc);platform_request_quit();}nds_perf_end_frame();frames_run++;
...
cleanup_mesh: nds_mesh_cache_destroy(mesh_cache);nds_draw_list_destroy(&draw_list);
