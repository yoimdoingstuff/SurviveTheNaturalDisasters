...
nds_result nds_app_run(const nds_app_options* options)
{
    ...
    {
        size_t initial_map=nds_map_catalog_select(1); const char* initial_path=options->map_path&&options->map_path[0]?options->map_path:nds_map_catalog_path(initial_map);
        rc=load_startup_scene(initial_path,mesh_cache,texture_cache,&scene);
    }
