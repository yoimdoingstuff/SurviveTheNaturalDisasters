...
void platform_set_window_title(const char* title){if(g_hwnd)SetWindowTextA(g_hwnd,title?title:"NDS Runtime");}
...
void platform_gl_context_destroy(void){if(g_gl_context){wglMakeCurrent(NULL,NULL);wglDeleteContext(g_gl_context);g_gl_context=NULL;}}nds_result platform_gl_context_make_current(void){return g_hdc&&g_gl_context&&wglMakeCurrent(g_hdc,g_gl_context)?NDS_OK:NDS_ERR_INIT_FAILED;}void platform_gl_swap_buffers(void){if(g_hdc&&g_gl_context)SwapBuffers(g_hdc);}void* platform_gl_get_proc_address(const char* name){if(!name)return NULL;PROC proc=wglGetProcAddress(name);if(proc)return(void*)proc;HMODULE module=GetModuleHandleA("opengl32.dll");return module?(void*)GetProcAddress(module,name):NULL;}nds_result platform_audio_init(void){return NDS_ERR_NOT_IMPLEMENTED;}void platform_audio_shutdown(void){}nds_result platform_socket_init(void){return NDS_ERR_NOT_IMPLEMENTED;}void platform_socket_shutdown(void){}
