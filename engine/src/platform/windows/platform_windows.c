...
{
    switch (msg) {
        case WM_CLOSE: g_quit_requested = 1; return 0;
        case WM_DESTROY: PostQuitMessage(0); return 0;
        case WM_KEYDOWN:
        case WM_KEYUP: {
            platform_key key = win32_vk_to_platform_key((int)wparam);
            if (key != PLATFORM_KEY_UNKNOWN) g_key_state[key] = (msg == WM_KEYDOWN) ? 1 : 0;
            return 0;
        }
        case WM_MOUSEMOVE:
            g_mouse_x = GET_X_LPARAM(lparam); g_mouse_y = GET_Y_LPARAM(lparam); return 0;
...
nds_result platform_init(void)
{
    ...
    if (!QueryPerformanceFrequency(&g_perf_frequency)) return NDS_ERR_INIT_FAILED;

    WNDCLASSEXA wc;
    memset(&wc, 0, sizeof(wc));
    wc.cbSize = sizeof(wc); wc.style = CS_HREDRAW | CS_VREDRAW | CS_OWNDC;
    wc.lpfnWndProc = nds_wnd_proc; wc.hInstance = g_hinstance;
    wc.hCursor = LoadCursor(NULL, IDC_ARROW); wc.lpszClassName = NDS_WNDCLASS_NAME;
    if (!RegisterClassExA(&wc)) {
        DWORD error = GetLastError();
        if (error != ERROR_CLASS_ALREADY_EXISTS) return NDS_ERR_INIT_FAILED;
    }
...
{
    if (!desc || desc->width <= 0 || desc->height <= 0) return NDS_ERR_INVALID_ARG;
    DWORD style = WS_OVERLAPPEDWINDOW;
    if (!desc->resizable) style &= ~(DWORD)(WS_THICKFRAME | WS_MAXIMIZEBOX);
    RECT rect = {0, 0, desc->width, desc->height};
    AdjustWindowRect(&rect, style, FALSE);
    g_hwnd = CreateWindowExA(0, NDS_WNDCLASS_NAME, desc->title ? desc->title : "NDS Runtime", style,
        CW_USEDEFAULT, CW_USEDEFAULT, rect.right - rect.left, rect.bottom - rect.top,
        NULL, NULL, g_hinstance, NULL);
    if (!g_hwnd) return NDS_ERR_INIT_FAILED;
    g_hdc = GetDC(g_hwnd);
    if (!g_hdc) { platform_destroy_window(); return NDS_ERR_INIT_FAILED; }
...
{
    if (!g_hwnd) { if (out_width) *out_width = 0; if (out_height) *out_height = 0; return; }
...
int platform_is_key_down(platform_key key) { return key >= 0 && key < PLATFORM_KEY_COUNT ? g_key_state[key] : 0; }
void platform_get_mouse_position(int* out_x, int* out_y) { if (out_x) *out_x = g_mouse_x; if (out_y) *out_y = g_mouse_y; }
...
long platform_file_size(platform_file* file) {
    if (!file) return -1; FILE* f = (FILE*)file; long current = ftell(f);
    if (current < 0 || fseek(f, 0, SEEK_END) != 0) return -1;
    long size = ftell(f); fseek(f, current, SEEK_SET); return size;
}
void platform_file_close(platform_file* file) { if (file) fclose((FILE*)file); }
int platform_file_exists(const char* path) { DWORD attr = GetFileAttributesA(path); return attr != INVALID_FILE_ATTRIBUTES && !(attr & FILE_ATTRIBUTE_DIRECTORY); }
uint64_t platform_time_now_ns(void) { LARGE_INTEGER counter; QueryPerformanceCounter(&counter); return (uint64_t)((double)counter.QuadPart / (double)g_perf_frequency.QuadPart * 1000000000.0); }
...
void platform_gl_context_destroy(void) { if (g_gl_context) { wglMakeCurrent(NULL, NULL); wglDeleteContext(g_gl_context); g_gl_context = NULL; } }
nds_result platform_gl_context_make_current(void) { return g_hdc && g_gl_context && wglMakeCurrent(g_hdc, g_gl_context) ? NDS_OK : NDS_ERR_INIT_FAILED; }
void platform_gl_swap_buffers(void) { if (g_hdc && g_gl_context) SwapBuffers(g_hdc); }
void* platform_gl_get_proc_address(const char* name) {
    if (!name) return NULL; PROC proc = wglGetProcAddress(name); if (proc) return (void*)proc;
    HMODULE module = GetModuleHandleA("opengl32.dll"); return module ? (void*)GetProcAddress(module, name) : NULL;
}
