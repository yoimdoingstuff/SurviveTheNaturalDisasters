...
{ size_t i; for (i=0;i<count;++i) { free(entries[i].class_name); free(entries[i].name); free(entries[i].mesh_asset); } free(entries); }
...
nds_result nds_map_load_json_text(const char* text, nds_instance** out_root)
{
    ...
    /* Validate the complete ID domain before allocating any Instances. */
    for (i=0;i<count;++i) {
        size_t j;
        if (entries[i].id < 0 || entries[i].id >= (int)count) { free_entries(entries,count); return NDS_ERR_IO; }
        for (j=0;j<i;++j) if (entries[j].id == entries[i].id) { free_entries(entries,count); return NDS_ERR_IO; }
        if (entries[i].parent >= (int)count) { free_entries(entries,count); return NDS_ERR_IO; }
    }
...
{
    size_t i;
    if (!node || !cache) return NDS_ERR_INVALID_ARG;
    if (nds_instance_get_class(node) == NDS_CLASS_PART || nds_instance_get_class(node) == NDS_CLASS_SPAWN_POINT) {
        const char* path = NULL;
        if (nds_part_get_mesh_asset(node, &path) == NDS_OK && path && path[0]) {
            const nds_mesh* mesh = NULL;
            nds_result rc = nds_mesh_cache_get(cache, path, &mesh);
            if (rc != NDS_OK) return rc;
            if (nds_part_set_mesh(node, mesh) != NDS_OK) return NDS_ERR_UNKNOWN;
        }
    }
    for (i=0;i<nds_instance_get_child_count(node);++i) {
        nds_result rc = resolve_meshes_recursive(nds_instance_get_child(node,i), cache);
        if (rc != NDS_OK) return rc;
    }
    return NDS_OK;
}
...
{
    return resolve_meshes_recursive(root, cache);
}
