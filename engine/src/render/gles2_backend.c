...
nds_result nds_gles2_backend_draw_hud(nds_gles2_backend* b,const nds_hud_state* state){const size_t max_quads=8192u;nds_hud_vertex* vertices=NULL;GLushort* indices=NULL;nds_hud_vertex* text_vertices=NULL;GLushort* text_indices=NULL;size_t quads=0,text_quads=0;NDS_LOGI("gles2","HUD: entered draw_hud, allocating buffers");vertices=(nds_hud_vertex*)malloc(max_quads*4*sizeof(*vertices));indices=(GLushort*)malloc(max_quads*6*sizeof(*indices));text_vertices=(nds_hud_vertex*)malloc(max_quads*4*sizeof(*text_vertices));text_indices=(GLushort*)malloc(max_quads*6*sizeof(*text_indices));if(!vertices||!indices||!text_vertices||!text_indices){NDS_LOGE("gles2","HUD: allocation failed");free(vertices);free(indices);free(text_vertices);free(text_indices);return NDS_ERR_UNKNOWN;}NDS_LOGI("gles2","HUD: buffers allocated");float health=0.0f,timer=0.0f,warning=0.0f;uint32_t round_number=0;nds_mat4 ortho;if(!b||!state)return NDS_ERR_INVALID_ARG;if(state->player){health=state->player->health/100.0f;if(health<0)health=0;if(health>1)health=1;}if(state->round){timer=1.0f-nds_round_phase_progress(state->round);round_number=state->round->round_number;}if(state->disasters&&nds_disaster_is_warning(state->disasters)){warning=nds_disaster_warning_remaining(state->disasters)/3.0f;if(warning<0)warning=0;if(warning>1)warning=1;}ortho.m[0]=2.0f/(float)b->width;ortho.m[1]=0;ortho.m[2]=0;ortho.m[3]=0;ortho.m[4]=0;ortho.m[5]=-2.0f/(float)b->height;ortho.m[6]=0;ortho.m[7]=0;ortho.m[8]=0;ortho.m[9]=0;ortho.m[10]=-1;ortho.m[11]=0;ortho.m[12]=-1;ortho.m[13]=1;ortho.m[14]=0;ortho.m[15]=1;b->gl.glUseProgram(b->program);glDisable(GL_DEPTH_TEST);glEnable(GL_BLEND);glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);b->gl.glUniformMatrix4fv(b->mvp_uniform,1,GL_FALSE,ortho.m);b->gl.glUniform1i(b->use_texture_uniform,0);NDS_LOGI("gles2","HUD: entering menu branch");if(state->menu_active){float w=(float)b->width,h=(float)b->height;float panel_w=w<720?w-48:720.0f;float panel_h=h<620?h-48:500.0f;float px=(w-panel_w)*0.5f,py=(h-panel_h)*0.5f;float s=w<900?2.0f:2.5f;const char* map_name=nds_map_catalog_name(nds_map_catalog_selected());NDS_LOGI("gles2","HUD: menu geometry start");hud_quad(vertices,indices,&quads,0,0,w,h);hud_quad(vertices,indices,&quads,px,py,panel_w,panel_h);hud_quad(vertices,indices,&quads,px+10,py+10,panel_w-20,6);hud_quad(vertices,indices,&quads,px+28,py+86,panel_w-56,104);hud_quad(vertices,indices,&quads,px+28,py+208,panel_w-56,78);hud_quad(vertices,indices,&quads,px+28,py+306,panel_w-56,62);hud_quad(vertices,indices,&quads,px+28,py+390,panel_w-56,70);NDS_LOGI("gles2","HUD: drawing menu background quads=%zu",quads);hud_draw_batch(b,vertices,indices,quads,0.015f,0.025f,0.045f,0.90f);NDS_LOGI("gles2","HUD: menu background drawn");text_quads=0;NDS_LOGI("gles2","HUD: building menu text");hud_text(text_vertices,text_indices,&text_quads,px+42,py+34,s,"NATURAL DISASTER SURVIVAL");hud_text(text_vertices,text_indices,&text_quads,px+48,py+104,1.5f,"PLAY");hud_text(text_vertices,text_indices,&text_quads,px+48,py+126,1.25f,"ENTER TO START");hud_text(text_vertices,text_indices,&text_quads,px+48,py+226,1.5f,"SELECT MAP");hud_text(text_vertices,text_indices,&text_quads,px+48,py+254,1.8f,"<");hud_text(text_vertices,text_indices,&text_quads,px+78,py+254,1.7f,map_name?map_name:"UNKNOWN MAP");hud_text(text_vertices,text_indices,&text_quads,px+panel_w-70,py+254,1.8f,">");hud_text(text_vertices,text_indices,&text_quads,px+48,py+322,1.35f,"I IMPORT ROBLOX MAP");hud_text(text_vertices,text_indices,&text_quads,px+48,py+406,1.1f,"WASD MOVE  SPACE JUMP");hud_text(text_vertices,text_indices,&text_quads,px+48,py+426,1.1f,"RMB CAMERA  C THIRD PERSON");hud_text(text_vertices,text_indices,&text_quads,px+48,py+448,1.1f,"M MENU");NDS_LOGI("gles2","HUD: drawing menu text quads=%zu",text_quads);hud_draw_batch(b,text_vertices,text_indices,text_quads,0.88f,0.96f,1.0f,0.96f);NDS_LOGI("gles2","HUD: menu text drawn");}else{hud_quad(vertices,indices,&quads,24,24,260,30);hud_quad(vertices,indices,&quads,28,28,252,22*health);hud_quad(vertices,indices,&quads,24,h-54,260,30);hud_quad(vertices,indices,&quads,28,h-50,252*timer,22);if(warning>0){hud_quad(vertices,indices,&quads,w*0.5f-180,48,360,42);hud_quad(vertices,indices,&quads,w*0.5f-174,54,348*warning,30);}for(int i=0;i<10;++i)if(i<(int)(round_number%10))hud_quad(vertices,indices,&quads,w-210+i*18,24,14,14);if(!state->player||!state->player->alive)hud_quad(vertices,indices,&quads,w*0.5f-120,h-90,240,48);hud_draw_batch(b,vertices,indices,quads,0.02f,0.03f,0.04f,0.78f);hud_draw_batch(b,vertices,indices,1,0,0,0,0);text_quads=0;hud_text(text_vertices,text_indices,&text_quads,30,31,1.0f,"HEALTH");hud_text(text_vertices,text_indices,&text_quads,30,h-48,1.0f,"SURVIVAL");if(warning>0)hud_text(text_vertices,text_indices,&text_quads,w*0.5f-105,62,1.0f,"WARNING");hud_text(text_vertices,text_indices,&text_quads,w-188,26,0.9f,"ROUND");if(!state->player||!state->player->alive)hud_text(text_vertices,text_indices,&text_quads,w*0.5f-77,h-77,1.2f,"ELIMINATED");hud_draw_batch(b,text_vertices,text_indices,text_quads,0.92f,0.96f,1.0f,0.95f);}glDisable(GL_BLEND);glEnable(GL_DEPTH_TEST);glDepthMask(GL_TRUE);b->gl.glDisableVertexAttribArray((GLuint)b->position_attrib);b->gl.glDisableVertexAttribArray((GLuint)b->uv_attrib);b->gl.glBindBuffer(GL_ARRAY_BUFFER,b->vertex_buffer);b->gl.glBufferData(GL_ARRAY_BUFFER,(GLsizeiptr)sizeof(cube_vertices),cube_vertices,GL_STATIC_DRAW);b->gl.glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,b->index_buffer);b->gl.glBufferData(GL_ELEMENT_ARRAY_BUFFER,(GLsizeiptr)sizeof(cube_indices),cube_indices,GL_STATIC_DRAW);b->gl.glUniform1i(b->use_texture_uniform,0);free(vertices);free(indices);free(text_vertices);free(text_indices);return NDS_OK;}
