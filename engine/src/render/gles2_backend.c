...
nds_result nds_gles2_backend_draw_hud(nds_gles2_backend* b,const nds_hud_state* state){nds_hud_vertex vertices[96];GLushort indices[144];size_t quads=0;float health=0.0f,timer=0.0f,warning=0.0f;uint32_t round_number=0;int i;nds_mat4 ortho;if(!b||!state)return NDS_ERR_INVALID_ARG;if(state->player){health=state->player->health/100.0f;if(health<0)health=0;if(health>1)health=1;}if(state->round){timer=1.0f-nds_round_phase_progress(state->round);round_number=state->round->round_number;}if(state->disasters&&nds_disaster_is_warning(state->disasters)){warning=nds_disaster_warning_remaining(state->disasters)/3.0f;if(warning<0)warning=0;if(warning>1)warning=1;}ortho.m[0]=2.0f/(float)b->width;ortho.m[1]=0;ortho.m[2]=0;ortho.m[3]=0;ortho.m[4]=0;ortho.m[5]=-2.0f/(float)b->height;ortho.m[6]=0;ortho.m[7]=0;ortho.m[8]=0;ortho.m[9]=0;ortho.m[10]=-1;ortho.m[11]=0;ortho.m[12]=-1;ortho.m[13]=1;ortho.m[14]=0;ortho.m[15]=1;hud_quad(vertices,indices,&quads,24,24,260,30);hud_quad(vertices,indices,&quads,28,28,252,22*health);hud_quad(vertices,indices,&quads,24,(float)b->height-54,260,30);hud_quad(vertices,indices,&quads,28,(float)b->height-50,252*timer,22);if(warning>0)hud_quad(vertices,indices,&quads,(float)b->width*0.5f-180,48,360,42),hud_quad(vertices,indices,&quads,(float)b->width*0.5f-174,54,348*warning,30);for(i=0;i<10;++i)if(i<(int)(round_number%10))hud_quad(vertices,indices,&quads,(float)b->width-210+i*18,24,14,14);if(!state->player||!state->player->alive)hud_quad(vertices,indices,&quads,(float)b->width*0.5f-120,(float)b->height-90,240,48);b->gl.glUseProgram(b->program);b->gl.glBindBuffer(GL_ARRAY_BUFFER,b->vertex_buffer);b->gl.glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,b->index_buffer);b->gl.glBufferData(GL_ARRAY_BUFFER,(GLsizeiptr)(quads*4*sizeof(vertices[0])),vertices,GL_STREAM_DRAW);b->gl.glBufferData(GL_ELEMENT_ARRAY_BUFFER,(GLsizeiptr)(quads*6*sizeof(indices[0])),indices,GL_STREAM_DRAW);b->gl.glVertexAttribPointer((GLuint)b->position_attrib,2,GL_FLOAT,GL_FALSE,(GLsizei)sizeof(nds_hud_vertex),(const void*)0);b->gl.glVertexAttribPointer((GLuint)b->uv_attrib,2,GL_FLOAT,GL_FALSE,(GLsizei)sizeof(nds_hud_vertex),(const void*)(2*sizeof(float)));b->gl.glEnableVertexAttribArray((GLuint)b->position_attrib);b->gl.glEnableVertexAttribArray((GLuint)b->uv_attrib);b->gl.glUniformMatrix4fv(b->mvp_uniform,1,GL_FALSE,ortho.m);b->gl.glUniform1i(b->use_texture_uniform,0);glDisable(GL_DEPTH_TEST);glEnable(GL_BLEND);glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);b->gl.glUniform4f(b->color_uniform,0.02f,0.03f,0.04f,0.78f);glDrawElements(GL_TRIANGLES,6,GL_UNSIGNED_SHORT,(const void*)0);if(health>0){b->gl.glUniform4f(b->color_uniform,0.18f,0.78f,0.28f,0.95f);glDrawElements(GL_TRIANGLES,6,GL_UNSIGNED_SHORT,(const void*)(6*sizeof(GLushort)));}b->gl.glUniform4f(b->color_uniform,0.02f,0.03f,0.04f,0.78f);glDrawElements(GL_TRIANGLES,6,GL_UNSIGNED_SHORT,(const void*)(12*sizeof(GLushort)));if(timer>0){b->gl.glUniform4f(b->color_uniform,0.25f,0.55f,0.95f,0.95f);glDrawElements(GL_TRIANGLES,6,GL_UNSIGNED_SHORT,(const void*)(18*sizeof(GLushort)));}if(warning>0){b->gl.glUniform4f(b->color_uniform,0.95f,0.65f,0.08f,0.95f);glDrawElements(GL_TRIANGLES,6,GL_UNSIGNED_SHORT,(const void*)(24*sizeof(GLushort)));b->gl.glUniform4f(b->color_uniform,0.95f,0.25f,0.08f,0.95f);glDrawElements(GL_TRIANGLES,6,GL_UNSIGNED_SHORT,(const void*)(30*sizeof(GLushort)));}{size_t base=warning>0?36:24;for(i=0;i<10;++i)if(i<(int)(round_number%10)){b->gl.glUniform4f(b->color_uniform,0.35f,0.75f,0.95f,0.95f);glDrawElements(GL_TRIANGLES,6,GL_UNSIGNED_SHORT,(const void*)((base+i*6)*sizeof(GLushort)));}if(!state->player||!state->player->alive){size_t dead_base=base+6*(round_number%10);b->gl.glUniform4f(b->color_uniform,0.85f,0.12f,0.12f,0.85f);glDrawElements(GL_TRIANGLES,6,GL_UNSIGNED_SHORT,(const void*)(dead_base*sizeof(GLushort)));}}glDisable(GL_BLEND);glEnable(GL_DEPTH_TEST);glDepthMask(GL_TRUE);b->gl.glDisableVertexAttribArray((GLuint)b->position_attrib);b->gl.glDisableVertexAttribArray((GLuint)b->uv_attrib);b->gl.glBindBuffer(GL_ARRAY_BUFFER,b->vertex_buffer);b->gl.glBufferData(GL_ARRAY_BUFFER,(GLsizeiptr)sizeof(cube_vertices),cube_vertices,GL_STATIC_DRAW);b->gl.glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,b->index_buffer);b->gl.glBufferData(GL_ELEMENT_ARRAY_BUFFER,(GLsizeiptr)sizeof(cube_indices),cube_indices,GL_STATIC_DRAW);b->gl.glUniform1i(b->use_texture_uniform,0);return NDS_OK;}
