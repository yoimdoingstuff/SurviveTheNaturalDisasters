...
static int test_valid_map(void)
{
    ...
    nds_result rc = nds_map_load_json_text(json, &root);
    if (rc != NDS_OK || !root) { fprintf(stderr, "map_loader_test: valid map load failed: %d\n", rc); return 1; }
    part = nds_instance_find_child(root, "Island");
    if (!part || nds_part_get_properties(part, &props) != NDS_OK) { nds_instance_destroy(root); fprintf(stderr, "map_loader_test: part lookup failed\n"); return 2; }
    if (props.position.x != 1.0f || props.position.y != 2.0f || props.position.z != 3.0f) { nds_instance_destroy(root); fprintf(stderr, "map_loader_test: position mismatch\n"); return 3; }
    if (props.size.x != 10.0f || props.size.y != 2.0f || props.size.z != 8.0f) { nds_instance_destroy(root); fprintf(stderr, "map_loader_test: size mismatch\n"); return 4; }
    if (props.transparency != 0.25f || props.reflectance != 0.1f || !props.anchored || !props.can_collide) { nds_instance_destroy(root); fprintf(stderr, "map_loader_test: part property mismatch\n"); return 5; }
    if (props.color_rgba != 0x7f3fffFFu && props.color_rgba != 0x7f40ffffu) { nds_instance_destroy(root); fprintf(stderr, "map_loader_test: color mismatch: 0x%08x\n", props.color_rgba); return 6; }
...
static int test_mesh_resolution(void)
{
    ...
    nds_result rc = nds_map_load_json_text(json, &root);
    if (rc != NDS_OK || !root) { fprintf(stderr, "map_loader_test: mesh map load failed: %d\n", rc); return 1; }
    part = nds_instance_find_child(root, "Triangle"); if (!part) { nds_instance_destroy(root); fprintf(stderr, "map_loader_test: mesh part lookup failed\n"); return 2; }
    if (nds_mesh_cache_create(&cache, 4) != NDS_OK) { nds_instance_destroy(root); return 3; }
    rc = nds_map_resolve_meshes(root, cache);
    if (rc != NDS_OK) { nds_mesh_cache_destroy(cache); nds_instance_destroy(root); fprintf(stderr, "map_loader_test: mesh resolve failed: %d\n", rc); return 4; }
    if (nds_part_get_mesh(part, &mesh) != NDS_OK || !mesh || mesh->vertex_count != 3 || mesh->index_count != 3) { nds_mesh_cache_destroy(cache); nds_instance_destroy(root); fprintf(stderr, "map_loader_test: resolved mesh mismatch\n"); return 5; }
    if (nds_mesh_cache_count(cache) != 1) { nds_mesh_cache_destroy(cache); nds_instance_destroy(root); fprintf(stderr, "map_loader_test: mesh cache count mismatch\n"); return 6; }
...
static int test_texture_resolution(void)
{
    ...
    nds_result rc = nds_map_load_json_text(json, &root);
    if (rc != NDS_OK || !root) { fprintf(stderr, "map_loader_test: texture map load failed: %d\n", rc); return 1; }
    part = nds_instance_find_child(root, "Textured"); if (!part) { nds_instance_destroy(root); fprintf(stderr, "map_loader_test: texture part lookup failed\n"); return 2; }
    if (nds_texture_cache_create(&cache, 4) != NDS_OK) { nds_instance_destroy(root); return 3; }
    rc = nds_map_resolve_textures(root, cache);
    if (rc != NDS_OK) { nds_texture_cache_destroy(cache); nds_instance_destroy(root); fprintf(stderr, "map_loader_test: texture resolve failed: %d\n", rc); return 4; }
    if (nds_part_get_texture(part, &texture) != NDS_OK || !texture || texture->width != 2 || texture->height != 2 || !texture->rgba8) { nds_texture_cache_destroy(cache); nds_instance_destroy(root); fprintf(stderr, "map_loader_test: resolved texture mismatch\n"); return 5; }
    if (nds_texture_cache_count(cache) != 1) { nds_texture_cache_destroy(cache); nds_instance_destroy(root); fprintf(stderr, "map_loader_test: texture cache count mismatch\n"); return 6; }
